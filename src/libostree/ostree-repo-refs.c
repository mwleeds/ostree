                                  ...
                                  GError                     **error)
{
  ...
  for (char **iter = (char **)refs_dirs->pdata; iter && *iter; iter++)
    {
      ...
      while (refs_dir_exists)
        {
          struct dirent *dent;
          g_autofree gchar *current_collection_id = NULL;
          ...
          if (g_strcmp0 (refs_dir, "refs/mirrors") == 0)
            {
              if (match_collection_id != NULL && g_strcmp0 (match_collection_id, dent->d_name) != 0)
                ...
                current_collection_id = g_strdup (dent->d_name);
            }
          else /* refs_dir = "refs/remotes" */
            {
              ...
              if (match_collection_id != NULL && g_strcmp0 (match_collection_id, remote_collection_id) != 0)
                ...
                current_collection_id = g_strdup (remote_collection_id);
            }

          if (!enumerate_refs_recurse (self, NULL, flags,
                                       current_collection_id, dfd_iter.fd, base_path,
                                       dfd_iter.fd, ".",
                                       ret_all_refs,
                                       cancellable, error))
            return FALSE;
